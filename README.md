...
| `new_config(name, count)` | `name = "World", count = 1` | `RandConfig` | Constructs a new configuration struct. |
| `core_format_greeting(name)` | `name` | `string` | Core formatter producing `Hello, {name}!`. |
| `core_format_custom(config)` | `config: RandConfig` | `string` | Formats greeting using prefix and name from config. |
